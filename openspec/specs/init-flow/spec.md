## Purpose
Первичная настройка проекта из панели: установка OpenSpec CLI, конфигурация проекта и деривация спецификаций с отслеживанием прогресса через чекпоинты, маркер незавершённой настройки и возобновление прерванного хода.
...
Промпт инициализации SHALL описывать уборку после отмены установки CLI механически проверяемыми шагами, не требуя от агента знания о том, что существовало до начала хода. После отмены финальная секция не выполняется – маркер снимается на этапе уборки.
