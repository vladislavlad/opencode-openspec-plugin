...
Система SHALL запрашивать `latest` у реестра npm для плагина и CLI, кодируя `/` в scoped-именах как `%2F`. Запросы SHALL выполняться параллельно, обрываться по таймауту в 3 секунды и при любой ошибке давать `null`, чтобы медленный или недоступный реестр не блокировал панель. Результат SHALL нести отдельный признак достижимости реестра, отличающий «всё свежее» от «проверить не удалось». Проверка SHALL выполняться один раз на рабочую директорию и по кнопке Check Versions, но не на опросе.
...
Промпт завершения обновления SHALL называть диапазон версий, затем перечислять шаги миграций по версиям, затем — release notes для пересказа пользователю. Требование снять `plugin.update-in-progress` SHALL добавляться только тогда, когда диапазон пришёл из флага.

#### Scenario: В диапазоне есть шаги
- **WHEN** хотя бы у одной записи диапазона непустые `instructions`
- **THEN** промпт содержит их по порядку, сгруппированные по версиям

#### Scenario: Шагов нет, есть только release notes
- **WHEN** у всех записей диапазона `instructions` пустые
- **THEN** промпт прямо сообщает, что шагов миграции нет, и переходит к пересказу release notes

#### Scenario: Диапазон из флага
- **WHEN** диапазон получен из `plugin.update-in-progress`
- **THEN** промпт заканчивается требованием удалить этот блок, сохранив остальное содержимое config.yaml
