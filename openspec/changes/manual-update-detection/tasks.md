...
- [ ] 5.5 Смоук на первом настоящем релизе (0.3.0): обновиться, убедиться что баннер появился, Complete Update отработал и повторно баннер не всплывает. В dev-чекауте не проверить — там `VERSION` равна `"dev"` и ветка `kv` неактивна по построению
- [ ] 5.6 Там же проверить путь через кнопку Update: флаг в config.yaml → Complete Update → агент снял флаг
