...
- [ ] 7.4 Отображение баннера/раздельных Update/Update All подтверждено визуально (превью через временный хардкод). Осталось прогнать вживую: блокировку кнопок при `busy()`, полный round-trip Update → reload → Complete Update на реальном обновлении
